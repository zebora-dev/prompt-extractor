...
def extract_chatgpt_batch_task(
    *,
    ...
    task_logger.info(
        "Starting ChatGPT extraction task. batch_id=%s prompts_file=%s limit=%s skip=%s dry_run=%s force_rerun=%s llm_model_filter=%s auto_login=%s login_email=%s capture_products=%s capture_entities=%s measurements_filter=%s max_prompts_per_session=%s",
        batch_id,
        prompts_file,
        limit,
        skip,
...
def extract_google_ai_mode_batch_task(
    *,
    ...
    task_logger = get_run_logger()
    task_logger.info(
        "Starting Google AI Mode extraction task. batch_id=%s prompts_file=%s limit=%s skip=%s dry_run=%s force_rerun=%s llm_model_filter=%s country=%s language=%s use_proxy=%s measurements_filter=%s",
        batch_id,
        prompts_file,
        limit,
        skip,
...
def _extract_google_ai_overview_batch_task_run_name() -> str:
    try:
        from prefect.runtime import task_run

        params = getattr(task_run, "parameters", None) or {}
        batch_id = params.get("batch_id") or "local-prompts"
        return f"extract-google-ai-overview-{batch_id}"
    except Exception as exc:
        LOGGER.debug("Prefect runtime not available: %s", exc)
        return "extract-google-ai-overview-batch"


@task(
    name="extract-google-ai-overview-batch",
    task_run_name=_extract_google_ai_overview_batch_task_run_name,
    retries=0,
    timeout_seconds=None,
    tags=["google", "ai-overview", "extraction", "browser"],
    cache_result_in_memory=False,
)
def extract_google_ai_overview_batch_task(
    *,
    batch_id: str | None = None,
    prompts_file: str | None = None,
    brand_id: str | None = None,
    ...
    if result.failed_count:
        task_logger.warning("Google AI Overview extraction completed with %s failed prompt(s).", result.failed_count)
    return payload


def _extract_claude_batch_task_run_name() -> str:
    try:
        from prefect.runtime import task_run

        params = getattr(task_run, "parameters", None) or {}
        batch_id = params.get("batch_id") or "local-prompts"
        return f"extract-claude-{batch_id}"
    except Exception as exc:
        LOGGER.debug("Prefect runtime not available: %s", exc)
        return "extract-claude-batch"


@task(
    name="extract-claude-batch",
    task_run_name=_extract_claude_batch_task_run_name,
    retries=0,
    timeout_seconds=None,
    tags=["claude", "extraction", "browser"],
    cache_result_in_memory=False,
)
def extract_claude_batch_task(
    *,
    batch_id: str | None = None,
    prompts_file: str | None = None,
    brand_id: str | None = None,
    ...
    if result.failed_count:
        task_logger.warning("Claude extraction completed with %s failed prompt(s).", result.failed_count)
    return payload


def _extract_perplexity_batch_task_run_name() -> str:
    try:
        from prefect.runtime import task_run

        params = getattr(task_run, "parameters", None) or {}
        batch_id = params.get("batch_id") or "local-prompts"
        return f"extract-perplexity-{batch_id}"
    except Exception as exc:
        LOGGER.debug("Prefect runtime not available: %s", exc)
        return "extract-perplexity-batch"


@task(
    name="extract-perplexity-batch",
    task_run_name=_extract_perplexity_batch_task_run_name,
    retries=0,
    timeout_seconds=None,
    tags=["perplexity", "extraction", "browser"],
    cache_result_in_memory=False,
)
def extract_perplexity_batch_task(
    *,
    batch_id: str | None = None,
    prompts_file: str | None = None,
    brand_id: str | None = None,
    ...
    task_logger = get_run_logger()
    task_logger.info(
        "Starting Perplexity extraction task. batch_id=%s limit=%s measurements_filter=%s",
        batch_id,
        limit,
        measurements_filter or "any",
    )
