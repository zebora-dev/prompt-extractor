"""
dispatch_perplexity_loop.py — polling dispatch loop for Perplexity extraction batches.
...
Usage:
    python scripts/dispatch_perplexity_loop.py \\
        --batch-id 54bd94c7-d52a-41a6-92c8-f96f9e235168 \\
...
_WORK_POOL_FOR_REGION = {
    "uk": "prompt-extraction-perplexity-uk",
    "us": "prompt-extraction-perplexity-us",
}

_FLY_APP_FOR_REGION = {
    "uk": "prompt-extractor-perplexity-uk",
    "us": "prompt-extractor-perplexity-us",
}
...
def dispatch_workers(batch_id: str, region: str, worker_count: int,
    ...
    deployment_full = f"perplexity-extraction-batch/perplexity-extraction-batch{region_suffix}"
...
def run_loop(
    ...
    work_pool = _WORK_POOL_FOR_REGION.get(region, f"prompt-extraction-perplexity-{region}")
    batch_deployment = (
        f"perplexity-extraction-batch/perplexity-extraction-batch-{region}"
        if region != "us"
        else "perplexity-extraction-batch/perplexity-extraction-batch"
    )

    print(f"\n{'='*60}")
    print(f"Perplexity dispatch loop")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Perplexity extraction dispatch loop")
