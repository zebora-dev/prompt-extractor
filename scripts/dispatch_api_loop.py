"""
dispatch_api_loop.py — polling dispatch loop for LLM API extraction batches.

Polls every N seconds:
  1. Counts remaining prompts (optionally filtered by measurements_filter and model).
...
_WORK_POOL_FOR_REGION = {
    "uk": "prompt-extraction-api-uk",
    "us": "prompt-extraction-api-us",
}

_FLY_APP_FOR_REGION = {
    "uk": "prompt-extractor-api-uk",
    "us": "prompt-extractor-api-us",
}
...
def dispatch_workers(
    ...
    worker_count: int,
    limit: int,
    measurements_filter: str | None,
    model_name: str,
    use_web_search: bool,
) -> list[str]:
    region_suffix = f"-{region}" if region != "us" else ""
    deployment_full = f"api-extraction-batch/api-extraction-batch{region_suffix}"
...
def run_loop(
    ...
    work_pool = _WORK_POOL_FOR_REGION.get(region, f"prompt-extraction-api-{region}")
    batch_deployment = (
        f"api-extraction-batch/api-extraction-batch-{region}"
        if region != "us"
        else "api-extraction-batch/api-extraction-batch"
    )
    effective_filter = f"api:{model_name}"

    print(f"\n{'='*60}")
    print(f"API dispatch loop")
    print(f"  batch_id           : {batch_id}")
    print(f"  model              : {model_name}")
    print(f"  use_web_search     : {use_web_search}")
    print(f"  llm_model_filter   : {effective_filter}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LLM API extraction dispatch loop")
    ...
    parser.add_argument("--limit", type=int, default=10, help="Prompts per inner run")
